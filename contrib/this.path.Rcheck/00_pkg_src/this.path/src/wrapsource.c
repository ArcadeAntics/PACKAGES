...
SEXP insidesource(SEXP call, SEXP op, SEXP args, SEXP rho, const char *name, Rboolean unset, SEXP fun_name)
{
    int nprotect = 0;
...
SEXP do_insidesource do_formals
{
    ...
    static SEXP fun_name = NULL;
    if (fun_name == NULL) {
        fun_name = mkChar("inside.source from package this.path");
    }


    return insidesource(call, op, args, rho, "inside.source", FALSE, fun_name);
}
...
SEXP do_setthispath do_formals
{
    ...
    static SEXP fun_name = NULL;
    if (fun_name == NULL) {
        fun_name = mkChar("set.this.path from package this.path");
    }


    return insidesource(call, op, args, rho, "set.this.path", FALSE, fun_name);
}
